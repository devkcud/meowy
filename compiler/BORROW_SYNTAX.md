...
remain unchanged. Root/compiler STATUS record the current stage and actual evidence.
...
`(&(*pointer)).field`, and `&!*pointer` remains an exclusive reborrow. Ordinary
`*pointer.field` retains its existing `*(pointer.field)` meaning. Parentheses reset
the boundary: `&(*pointer.field)` borrows the dereferenced selected value.
...
bitwise/logical operators, task groups and all non-borrowing syntax keep their roles.
...
2. Extend `parser/expressions.rs` to lower dotted borrowing into existing
   `Unary(Field)` nodes. Give prefix borrowing a postfix boundary inherited by its
   nested prefixes, with grouping resetting it. Keep the existing AST/HIR, ownership
...
3. Preserve old program intent before qualifying the switch. Old full-place borrows
   become explicit groups (`&!(rows[index].field)`) or selected-field forms
   (`rows[index].&!field`). Do not turn those into borrows of the containing list.
...
4. Test native shared/exclusive field borrowing, grouped indexed borrowing,
   receiver-borrow selection, nested paths, sibling disjointness, conflicts,

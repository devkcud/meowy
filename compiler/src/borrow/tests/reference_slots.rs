...
#[test]
pub(crate) fn selected_carrier_addresses_keep_reference_storage_boundaries() {
    for source in [
        "owner:1;result:{->view:&owner;p:&view}",
        "owner:1;result:{->carrier:{->view:&owner;->n:2};p:&carrier}",
        "owner:1;result:{->carrier:{->view:&owner;->n:2};p:&carrier.view}",
        "owner:1;result:{->carrier:={->view:&owner;->n:2}}",

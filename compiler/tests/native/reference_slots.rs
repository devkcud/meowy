...
pub fn reference_slot_storage_keeps_whole_carrier_and_mutation_boundaries() {
    for source in [
        "owner:1;value:{->view:=&owner;view=&owner;n:=2;'loop{n=n-1;|n>0|'loop.restart()}}",
        "owner:1;value:{->holder:={->view:&owner;->count:3};holder={->view:&owner;->count:4};n:=2;'loop{n=n-1;|n>0|'loop.restart()}}",
